...
export default function PreviewCard({ file, onClear }: Props) {
  ...
  return (
    <div className="relative w-full rounded-2xl border border-zinc-800 overflow-hidden bg-zinc-900/50">
      {isPdf ? (
        <div className="flex items-center justify-center h-32 gap-3">
          <svg
            className="w-10 h-10 shrink-0 text-zinc-600"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.5}
            viewBox="0 0 24 24"
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-zinc-800">
        <span className="text-xs text-zinc-500 font-mono truncate max-w-[80%]">
          {file.name}
        </span>
        <button
          onClick={onClear}
