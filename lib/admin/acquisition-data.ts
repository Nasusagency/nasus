...
export async function getAdminLead(id: string) {
  ...
  const { data: lead } = await supabase.from("whatsapp_leads")
    .select("id,numero,nombre_contacto,nombre_empresa,sector,stage,problema_descrito,servicio_probable,resumen,requiere_humano,razon_handoff,ultima_interaccion,created_at,acquisition_event_id,acquisition_events(attribution_id,session_id,source,medium,campaign,content,term,landing_path,created_at)")
    .eq("id", id).maybeSingle();
  if (!lead) return null;
  const attribution: any = Array.isArray(lead.acquisition_events) ? lead.acquisition_events[0] : lead.acquisition_events;
  const [messages, requirements, firstMessage, sessionEvents, metricRows] = await Promise.all([
  ...
  if (rows.length > 0 && attribution?.source && attribution?.campaign) {
    ...
    const [campaignEventsResult, campaignLeadsResult] = await Promise.all([
      ...
      supabase.from("whatsapp_leads").select("numero,stage,acquisition_events!inner(source,campaign)").eq("acquisition_events.source", attribution.source).eq("acquisition_events.campaign", attribution.campaign).gte("created_at", rangeStart).lt("created_at", rangeEnd.toISOString()).limit(5000),
    ]);
    ...
    const owned = { visits: new Set(campaignEvents.filter(event => event.event_type === "page_view").map(event => event.session_id)).size, whatsappClicks: campaignEvents.filter(event => event.event_type === "whatsapp_click").length, conversations: new Set((campaignMessages.data ?? []).map(message => message.conversation_id)).size, leads: campaignLeads.length, qualified: campaignLeads.filter(item => item.stage === "qualified").length, highIntent: campaignLeads.filter(item => item.stage === "high_intent").length };
