...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="p-6 lg:p-8 max-w-7xl mx-auto">
    ...
    {leads.length ? <div className="overflow-x-auto border border-zinc-200 bg-white rounded-xl"><table className="w-full text-xs"><thead className="bg-zinc-50 text-zinc-600"><tr>{["Contacto", "Teléfono", "Empresa / sector", "Fuente / campaña", "Lifecycle", "Stage", "Señales", "Última interacción"].map(h => <th key={h} className="text-left font-medium px-3 py-3">{h}</th>)}</tr></thead><tbody>{leads.map(l => <tr key={l.id} className="border-t border-zinc-100 align-top hover:bg-amber-50/30"><td className="px-3 py-3"><Link href={`/admin/leads/${l.id}`} className="font-medium text-[#76582f]">{l.nombre_contacto || "Sin nombre"}</Link></td><td className="px-3 py-3 text-zinc-600">{maskPhone(l.numero)}</td><td className="px-3 py-3 text-zinc-700">{l.nombre_empresa || "—"}<br/><span className="text-zinc-500">{l.sector || "—"}</span></td><td className="px-3 py-3 text-zinc-700">{l.source || "Sin atribución"}<br/><span className="text-zinc-500">{l.campaign || "—"}</span></td><td className="px-3 py-3"><Badge tone={LIFECYCLE_TONE[l.lifecycle] ?? "neutral"}>{l.lifecycle}</Badge></td><td className="px-3 py-3"><Badge tone={STAGE_TONE[l.stage] ?? "neutral"}>{l.stage}</Badge></td><td className="px-3 py-3 space-y-1">{l.high_intent_detected_at && <Badge tone="warning">High intent</Badge>}{l.requiere_humano && <Badge tone="danger" className="block w-fit">Requiere humano</Badge>}</td><td className="px-3 py-3 text-zinc-500">{new Date(l.ultima_interaccion).toLocaleString("es-MX")}</td></tr>)}</tbody></table></div> : <EmptyState title="No hay contactos para estos filtros" description={stage || lifecycle || human ? "Ajusta o quita los filtros para ver más contactos." : "Los contactos aparecen aquí en cuanto alguien escribe por WhatsApp o se registra manualmente."} action={(stage || lifecycle || human) ? <Link href="/admin/leads" className="rounded-lg border border-zinc-300 px-4 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50">Quitar filtros</Link> : undefined} />}
  </div>;
}
